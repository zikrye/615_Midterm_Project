...
ggplot(df) + geom_freqpoly(aes(x = Value, color = Level), alpha= 100) + 
  ...
  labs(title = "Chemical use by toxicity category")
...
ggplot(df) + 
  ...
  coord_cartesian(ylim = c(0, 60))+
  labs(title = "Chemical use in California compared to all the state")
...
ggplot(df_Cal) + geom_freqpoly(aes(x = Value, color = Level), alpha= 100) + 
  ...
  facet_wrap(~Toxicity)+
  coord_cartesian(ylim = c(0, 60))+
  labs(title = "Chemical use by toxicity category in California")
